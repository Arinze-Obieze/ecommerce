...
export default function StoreDashboardTeamPage() {
  ...
  const updateInvitation = async (inviteId, action) => {
    try {
      ...
      setError('');
      setNotice('');
      const res = await fetch(`/api/store/team/invitations/${inviteId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-[#dbe7e0] bg-white p-5 shadow-sm">
        ...
        <p className="text-sm text-gray-500">
          Add teammates, invite people who have not signed up yet, and manage access for <span className="font-semibold text-gray-700">{store?.name || 'your store'}</span>.
        </p>
      ...
      {migrationHint ? <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">{migrationHint}</div> : null}
      ...
      <div className="rounded-2xl border border-[#dbe7e0] bg-white p-5 shadow-sm">
        ...
        <form onSubmit={addMember} className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <button
            type="submit"
            disabled={!canManageTeam || saving}
            className="rounded-xl bg-[#2E5C45] px-4 py-2 text-sm font-semibold text-white hover:bg-[#254a38] disabled:cursor-not-allowed disabled:opacity-50 md:w-fit"
          >
        ...
        {!canManageTeam ? (
          <p className="mt-3 text-xs text-amber-700">
            Only owner or manager can add or revoke team members.
          </p>
        ) : null}
      </div>
                  ...
                  <tr key={row.id} className="border-b border-gray-50">
                    ...
                    <td className="py-2 pr-3">
                      <div className="flex flex-wrap gap-2">
                        <button
                          type="button"
                          onClick={() => updateInvitation(row.id, 'resend')}
                        ...
                        <button
                          type="button"
                          onClick={() => updateInvitation(row.id, 'revoke')}

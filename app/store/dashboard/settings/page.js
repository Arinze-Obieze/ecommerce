  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
        <div className="rounded-xl border border-[#dbe7e0] bg-white p-4 shadow-sm"><p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 sm:text-xs">Role</p><p className="mt-1 text-sm font-semibold capitalize text-gray-900">{meta.role || '-'}</p></div>
        <div className="rounded-xl border border-[#dbe7e0] bg-white p-4 shadow-sm"><p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 sm:text-xs">Store Status</p><p className="mt-1 text-sm font-semibold capitalize text-gray-900">{snapshot?.status || '-'}</p></div>
        <div className="rounded-xl border border-[#dbe7e0] bg-white p-4 shadow-sm"><p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 sm:text-xs">KYC / Payout</p><p className="mt-1 break-words text-sm font-semibold text-gray-900">{(snapshot?.kyc_status || 'pending')} / {snapshot?.payout_ready ? 'Ready' : 'Not ready'}</p></div>
      </div>
      ...
      <form onSubmit={onSave} className="rounded-2xl border border-[#dbe7e0] bg-white p-5 shadow-sm">
            ...
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-3">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <label className="cursor-pointer rounded-xl border border-[#2E5C45] px-3 py-2 text-sm font-semibold text-[#2E5C45] transition hover:bg-[#f3f8f5] disabled:opacity-60">
        ...
        <div className="mt-4 flex items-center gap-3">
          <button type="submit" disabled={!meta.can_edit || !changed || saving || uploadingLogo} className="rounded-xl bg-[#2E5C45] px-4 py-2 text-sm font-semibold text-white hover:bg-[#254a38] disabled:opacity-60">
            {saving ? 'Saving...' : 'Save Settings'}
          </button>
          <button type="button" onClick={load} disabled={saving} className="rounded-xl border border-[#2E5C45] px-4 py-2 text-sm font-semibold text-[#2E5C45] disabled:opacity-60">
            Refresh
          </button>
        </div>
      </form>

...
export default function AdminAnalyticsPage() {
  ...
  if (loading) return <div className="rounded-2xl border border-[#E8E4DC] bg-white p-6">Loading analytics...</div>;
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-[#E8E4DC] bg-white p-4 shadow-sm"><p className="text-xs uppercase text-gray-500">DAU</p><p className="mt-2 text-2xl font-bold">{acquisition.dau}</p></div>
        <div className="rounded-2xl border border-[#E8E4DC] bg-white p-4 shadow-sm"><p className="text-xs uppercase text-gray-500">WAU</p><p className="mt-2 text-2xl font-bold">{acquisition.wau}</p></div>
        <div className="rounded-2xl border border-[#E8E4DC] bg-white p-4 shadow-sm"><p className="text-xs uppercase text-gray-500">MAU</p><p className="mt-2 text-2xl font-bold">{acquisition.mau}</p></div>
      </div>
          ...
          <div className="h-60">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={acquisition.dauTrend}>
                ...
                <Tooltip />
                <Line type="monotone" dataKey="dau" stroke="#2E6417" strokeWidth={2.5} dot={{ r: 2 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-[#E8E4DC] bg-white p-5 shadow-sm">
          ...
          <div className="space-y-2 text-sm">
            {behavior.topSearchTerms.map((term) => (
              <div key={term.term} className="flex items-center justify-between rounded-lg bg-[#EDF5E6] px-3 py-2">
                <span>{term.term}</span>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        ...
        <div className="rounded-2xl border border-[#E8E4DC] bg-white p-5 shadow-sm">
          ...
          <div className="space-y-2 text-sm">
            {geography.ordersByState.map((row) => (
              <div key={row.state} className="flex justify-between rounded-lg bg-[#EDF5E6] px-3 py-2">
                <span>{row.state}</span>
        ...
        <div className="rounded-2xl border border-[#E8E4DC] bg-white p-5 shadow-sm">
          ...
          <div className="space-y-2 text-sm">
            {devices.map((row) => (
              <div key={row.device} className="flex justify-between rounded-lg bg-[#EDF5E6] px-3 py-2">
        ...
        <div className="rounded-2xl border border-[#E8E4DC] bg-white p-5 shadow-sm">
          ...
          <div className="space-y-2 text-sm">
            {recommendations.recommendationSurfaceMix.map((row) => (
              <div key={row.surface} className="flex justify-between rounded-lg bg-[#EDF5E6] px-3 py-2">
        ...
        <div className="rounded-2xl border border-[#E8E4DC] bg-white p-5 shadow-sm">
          ...
          <div className="space-y-2 text-sm">
            {recommendations.recommendationPersonaMix.map((row) => (
              <div key={row.persona} className="flex justify-between rounded-lg bg-[#EDF5E6] px-3 py-2">

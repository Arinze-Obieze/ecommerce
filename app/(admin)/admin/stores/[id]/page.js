...
export default function AdminStoreDetailPage() {
  ...
  if (loading) return <div className="rounded-2xl border border-[#E8E4DC] bg-white p-6">Loading store...</div>;
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-[#E8E4DC] bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 text-sm md:grid-cols-3">
          <div className="rounded-xl bg-[#EDF5E6] px-3 py-2">Status: <strong className="capitalize">{store.status}</strong></div>
          <div className="rounded-xl bg-[#EDF5E6] px-3 py-2">KYC: <strong className="capitalize">{store.kyc_status}</strong></div>
          <div className="rounded-xl bg-[#EDF5E6] px-3 py-2">Payout: <strong>{store.payout_ready ? 'Ready' : 'Not ready'}</strong></div>
        </div>
      </div>

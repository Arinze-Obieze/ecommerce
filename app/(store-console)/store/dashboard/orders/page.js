    ...
    <div className="relative" onClick={(event) => event.stopPropagation()}>
      ...
      {open ? (
        <div className="absolute right-0 z-20 mt-2 w-52 rounded-xl border border-[#E8E4DC] bg-white p-2 shadow-lg" role="menu">
          <Link href={`/store/dashboard/orders/${orderId}`} className="block rounded-lg px-3 py-2 text-sm text-gray-700 transition hover:bg-[#EDF5E6]" onClick={onClose}>
            Open full order
          </Link>
          <Link href={`/store/dashboard/orders/${orderId}#fulfillment-update`} className="mt-1 block rounded-lg px-3 py-2 text-sm text-gray-700 transition hover:bg-[#EDF5E6]" onClick={onClose}>
            Update fulfillment status
          </Link>
          <button type="button" onClick={() => { onOpenReceipt(orderId); onClose(); }} className="mt-1 block w-full rounded-lg px-3 py-2 text-left text-sm text-gray-700 transition hover:bg-[#EDF5E6]">
            Preview Zova receipt (PDF)
          </button>
          <button type="button" onClick={() => { onDownloadReceipt(orderId); onClose(); }} className="mt-1 block w-full rounded-lg px-3 py-2 text-left text-sm text-gray-700 transition hover:bg-[#EDF5E6]">
          ...
          <div className="rounded-2xl border border-[#E8E4DC] bg-white p-4 shadow-sm">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">
              <input
                ...
                className="rounded-xl border border-[#E8E4DC] bg-white px-3 py-2 text-sm text-gray-800 outline-none transition focus:border-[#2E6417] focus:ring-2 focus:ring-[#2E6417]/20 md:col-span-2 xl:col-span-2"
              />
              <select value={paymentFilter} onChange={(event) => setPaymentFilter(event.target.value)} className="rounded-xl border border-[#E8E4DC] bg-white px-3 py-2 text-sm text-gray-800 outline-none transition focus:border-[#2E6417] focus:ring-2 focus:ring-[#2E6417]/20">
                {PAYMENT_FILTERS.map((value) => <option key={value} value={value}>{value === 'all' ? 'All payment' : prettify(value)}</option>)}
              </select>
              <select value={fulfillmentFilter} onChange={(event) => setFulfillmentFilter(event.target.value)} className="rounded-xl border border-[#E8E4DC] bg-white px-3 py-2 text-sm text-gray-800 outline-none transition focus:border-[#2E6417] focus:ring-2 focus:ring-[#2E6417]/20">
                {FULFILLMENT_FILTERS.map((value) => <option key={value} value={value}>{value === 'all' ? 'All fulfillment' : prettify(value)}</option>)}
              </select>
              <select value={escrowFilter} onChange={(event) => setEscrowFilter(event.target.value)} className="rounded-xl border border-[#E8E4DC] bg-white px-3 py-2 text-sm text-gray-800 outline-none transition focus:border-[#2E6417] focus:ring-2 focus:ring-[#2E6417]/20">
                {ESCROW_FILTERS.map((value) => <option key={value} value={value}>{value === 'all' ? 'All escrow' : prettify(value)}</option>)}
              </select>
              <button type="button" onClick={() => { setQuery(''); setPaymentFilter('all'); setFulfillmentFilter('all'); setEscrowFilter('all'); }} className="rounded-xl border border-[#E8E4DC] px-3 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50">
                Reset
              </button>
            </div>
          </div>
                  ...
                  <table className="min-w-full text-sm">
                    ...
                    <tbody>
                      {filteredRows.map((row) => {
                        ...
                        return (
                          <tr key={row.id} className={`cursor-pointer border-b border-gray-50 transition ${active ? 'bg-[#EDF5E6]' : 'hover:bg-gray-50'}`} onClick={() => setSelectedOrderId(row.id)}>
                            <td className="py-2 pr-3 font-mono text-xs font-semibold text-[#2E6417]">{row.id.slice(0, 8)}</td>
        ...
        <aside className="hidden xl:block">
          <div className="sticky top-24 rounded-2xl border border-[#E8E4DC] bg-white p-4 shadow-sm">
            <div className="mb-3 flex items-center justify-between gap-2">
              ...
              {selectedOrderId ? (
                <Link href={`/store/dashboard/orders/${selectedOrderId}`} className="text-xs font-semibold text-[#2E6417] hover:underline">Open full page</Link>
              ) : null}
            </div>
            ...
            {detailOrder && !detailLoading ? (
              <div className="space-y-4 text-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  <Link href={`/store/dashboard/orders/${selectedOrderId}#fulfillment-update`} className="rounded-lg border border-[#E8E4DC] px-3 py-2 text-xs font-semibold text-gray-700 transition hover:bg-gray-50">Update fulfillment</Link>
                  <button type="button" onClick={() => openReceipt(selectedOrderId)} className="rounded-lg border border-[#2E6417] bg-[#EDF5E6] px-3 py-2 text-xs font-semibold text-[#2E6417] transition hover:bg-[#e2eee8]">Preview Zova PDF</button>
                </div>

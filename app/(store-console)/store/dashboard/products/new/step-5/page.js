              ...
              <ul className="mt-2 space-y-1 text-sm text-gray-700">
                {tiers.map((tier) => (
                  <li key={`${tier.minimum_quantity}-${tier.discount_percent}`}>
                    Buy <span className="font-semibold">{tier.minimum_quantity}+</span> → <span className="font-semibold text-primary">{tier.discount_percent}% off</span>
                  </li>
                ))}
              </ul>
            ) : (

...
const MODERATION_COLORS = ['#2E6417', '#0ea5e9', '#ef4444', '#f59e0b'];
const RANGE_OPTIONS = [
...
export default function StoreAnalyticsPage() {
  ...
  if (loading) return <div className="rounded-2xl border border-[#E8E4DC] bg-white p-6 text-sm text-gray-500">Loading analytics...</div>;
    ...
    <div className="flex flex-wrap gap-2">
      {RANGE_OPTIONS.map((option) => (
        <button
          ...
          className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${range === option.value ? 'bg-[#2E6417] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
        >
          {option.label}
        </button>
      ))}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-2 xl:grid-cols-5">
        <div className="rounded-2xl border border-[#E8E4DC] bg-white p-3 shadow-sm sm:p-4"><p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 sm:text-xs">Total Orders</p><p className="mt-2 break-words text-xl font-bold sm:text-2xl">{orders.totalOrders || 0}</p></div>
        <div className="rounded-2xl border border-[#E8E4DC] bg-white p-3 shadow-sm sm:p-4"><p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 sm:text-xs">Paid Orders</p><p className="mt-2 break-words text-xl font-bold sm:text-2xl">{orders.paidOrders || 0}</p></div>
        <div className="rounded-2xl border border-[#E8E4DC] bg-white p-3 shadow-sm sm:p-4"><p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 sm:text-xs">Gross Sales</p><p className="mt-2 break-words text-lg font-bold sm:text-2xl">{formatCurrency(orders.grossSales || 0)}</p></div>
        <div className="rounded-2xl border border-[#E8E4DC] bg-white p-3 shadow-sm sm:p-4"><p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 sm:text-xs">Products in Carts</p><p className="mt-2 break-words text-xl font-bold sm:text-2xl">{cartDemand.productsInCarts || 0}</p></div>
        <div className="rounded-2xl border border-[#E8E4DC] bg-white p-3 shadow-sm sm:p-4"><p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 sm:text-xs">Events Captured</p><p className="mt-2 break-words text-xl font-bold sm:text-2xl">{cartDemand.eventsCaptured || 0}</p></div>
      </div>
          ...
          <div className="mt-4 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={dailyRevenue}>
                ...
                <Line yAxisId="right" type="monotone" dataKey="revenue" stroke="#2E6417" strokeWidth={2.5} dot={{ r: 2 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="mt-4 h-60">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dailyCart}>
                ...
                <Bar dataKey="netUnits" fill="#2E6417" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>

    ...
    <div className="space-y-6">
      ...
      >
        <p className="text-xs text-gray-500">
          Current role: <span className="font-semibold text-[#2E6417]">{roleLabel(currentMembership?.role)}</span>
        </p>
      </DashboardPageHeader>
      ...
      <div className="rounded-2xl border border-[#E8E4DC] bg-white p-5 shadow-sm">
        ...
        <form onSubmit={addMember} className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <button type="submit" disabled={!canManageTeam || saving} className="rounded-xl bg-[#2E6417] px-4 py-2 text-sm font-semibold text-white hover:bg-[#245213] disabled:cursor-not-allowed disabled:opacity-50 md:w-fit">
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id} className="border-b border-gray-50">
                    ...
                    <td className="py-2 pr-3">
                      {row.role === 'owner' ? (
                        <span className="rounded-full bg-[#EDF5E6] px-2 py-1 text-xs font-semibold text-[#2E6417]">Owner</span>
                      ) : (
                    ...
                    <td className="py-2 pr-3">
                      {row.role === 'owner' ? (
                        <span className="text-xs font-semibold text-[#2E6417]">Active</span>
                      ) : (

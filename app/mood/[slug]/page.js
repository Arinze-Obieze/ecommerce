  ...
  return (
    <Link href={`/products/${product.slug}`} className="group block">
      <div className="relative rounded-2xl overflow-hidden bg-gray-100 aspect-[3/4] mb-3">
        ...
        {pct > 0 && (
          <span className="absolute top-2.5 left-2.5 bg-[#2E5C45] text-white text-[10px] font-black px-2 py-0.5 rounded-full">
            -{pct}%
          </span>
        )}
        ...
        <div className="absolute bottom-0 inset-x-0 p-3 translate-y-1 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-200">
          <button
            ...
            className="w-full bg-[#2E5C45] text-white text-xs font-bold py-2.5 rounded-xl hover:bg-[#254a38] transition-colors shadow-lg"
          >
            Quick Add
          </button>
        </div>
      ...
      <div className="space-y-1 px-0.5">
        ...
        <p className="text-sm font-semibold text-gray-900 leading-snug line-clamp-2 group-hover:text-[#2E5C45] transition-colors">
          {product.name}
        </p>
    ...
    <div className="border-b border-gray-100 last:border-0">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {badge > 0 && (
            <span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-[#2E5C45] text-white text-[9px] font-black">
              {badge}
            </span>
          )}
        </div>
...
function Sidebar({ sort, setSort, pricePreset, setPricePreset, selectedCategories, setSelectedCategories, selectedSizes, setSelectedSizes, selectedColors, setSelectedColors, availableCategories, availableSizes, availableColors, activeFilterCount, onClearAll }) {
  ...
  const CheckRow = ({ active, label, onClick }) => (
    <button onClick={onClick} className={`w-full flex items-center gap-3 px-2.5 py-2 rounded-xl text-sm transition-all ${active ? "bg-[#2E5C45]/6 text-[#2E5C45] font-semibold" : "text-gray-600 hover:bg-gray-50 hover:text-gray-800"}`}>
      <span className={`w-4 h-4 rounded-[5px] border flex-shrink-0 flex items-center justify-center transition-all ${active ? "bg-[#2E5C45] border-[#2E5C45]" : "border-gray-300"}`}>
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <div className="flex items-center gap-2">
          ...
          {activeFilterCount > 0 && (
            <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-[#2E5C45] text-white text-[10px] font-black">{activeFilterCount}</span>
          )}
        </div>
        {activeFilterCount > 0 && (
          <button onClick={onClearAll} className="text-xs font-semibold text-[#2E5C45] hover:text-[#254a38] transition-colors">
            Clear all
          </button>
        )}
      </div>
          ...
          <div className="space-y-0.5">
            {SORT_OPTIONS.map(opt => (
              <button key={opt.value} onClick={() => setSort(opt.value)}
                className={`w-full flex items-center justify-between px-2.5 py-2 rounded-xl text-sm transition-all ${sort === opt.value ? "bg-[#2E5C45]/6 text-[#2E5C45] font-semibold" : "text-gray-600 hover:bg-gray-50 hover:text-gray-800"}`}>
                {opt.label}
                {sort === opt.value && (
                  <span className="w-4 h-4 rounded-full bg-[#2E5C45] flex items-center justify-center flex-shrink-0">
              ...
              {availableSizes.map(sz => {
                ...
                return (
                  <button key={sz} onClick={() => toggleArr(setSelectedSizes, sz)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${active ? "border-[#2E5C45] bg-[#2E5C45] text-white shadow-sm" : "border-gray-200 text-gray-600 hover:border-[#2E5C45]/40 hover:text-[#2E5C45]"}`}>
                    {sz}
                  </button>
                );
              })}
              ...
              {availableColors.map(color => {
                ...
                return (
                  <button key={color} onClick={() => toggleArr(setSelectedColors, color)}
                    className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold border transition-all ${active ? "border-[#2E5C45] bg-[#2E5C45]/8 text-[#2E5C45]" : "border-gray-200 text-gray-500 hover:border-gray-300 hover:text-gray-700"}`}>
                    {color}
                  </button>
                );
              })}
...
function FilterChip({ label, onRemove }) {
  return (
    <span className="inline-flex items-center gap-1.5 pl-3 pr-2 py-1.5 rounded-full bg-[#2E5C45]/8 text-[#2E5C45] text-xs font-semibold border border-[#2E5C45]/15">
      {label}
      <button onClick={onRemove} className="w-3.5 h-3.5 rounded-full bg-[#2E5C45]/15 hover:bg-[#2E5C45] hover:text-white flex items-center justify-center transition-colors leading-none">×</button>
    </span>
  );
}
...
export default function MoodPage() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-white flex items-center justify-center">
      <div className="flex flex-col items-center gap-3">
        <div className="w-10 h-10 border-[3px] border-gray-100 border-t-[#2E5C45] rounded-full animate-spin" />
  ...
  if (error || !mood) return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-4">
      ...
      <Link href="/" className="text-sm text-[#2E5C45] font-semibold">← Back to home</Link>
    </div>
  );
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              >
                ...
                {activeFilters.length > 0 && (
                  <span className="w-5 h-5 rounded-full bg-[#00B86B] text-white text-[10px] font-black flex items-center justify-center">{activeFilters.length}</span>
                )}
              </button>
            </div>
          </div>
          ...
          <main className="flex-1 min-w-0">
            {filteredProducts.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-28 text-center">
                ...
                <button onClick={clearAllFilters} className="px-5 py-2.5 rounded-xl bg-[#2E5C45] text-white font-bold text-sm hover:bg-[#254a38] transition-colors">
                  Clear all filters
                </button>
              </div>
            ) : (

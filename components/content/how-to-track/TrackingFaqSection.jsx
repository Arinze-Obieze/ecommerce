...
import { TRACKING_FAQS } from "@/components/content/how-to-track/howToTrack.constants";

export default function TrackingFaqSection({ openFaq, setOpenFaq }) {
  return (
    <section style={{ marginBottom: 52 }}>
      <SectionHeader kicker="Questions?" title="Tracking FAQs" />

      <div className="zova-info-card" style={{ overflow: "hidden" }}>
        {TRACKING_FAQS.map((item, index) => {
            ...
            <div key={item.q} className="zova-info-faq-item" onClick={() => setOpenFaq(isOpen ? null : index)}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
                ...
                <div className={`zova-info-faq-toggle ${isOpen ? "is-open" : ""}`}>
                  <span style={{ fontSize: 14, transform: isOpen ? "rotate(45deg)" : "rotate(0deg)", display: "block", transition: "transform 0.2s", lineHeight: 1 }}>+</span>
                </div>
              </div>
              {isOpen ? (

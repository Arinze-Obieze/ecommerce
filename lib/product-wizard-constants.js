...
export const WIZARD_STEPS = [
  ...
  { num: 4, label: "Media",     slug: "step-4" },
  { num: 5, label: "Specs",     slug: "step-5" },
  { num: 6, label: "Review",    slug: "step-6" },
];
...
export function isWizardStepComplete(stepNum, state) {
  switch (stepNum) {
    case 1:
      return Boolean(state?.category && state?.subcategory);
    case 2:
      return Boolean(
    ...
    case 4:
      return hasValidImages(state);
    case 5:
      return hasSpecificationContent(state);
    case 6: {
      const validVariantCount = (state?.variants || []).filter((variant) => variant?.color && variant?.size).length;
      return Boolean(
        state?.baseSku &&
        Array.isArray(state?.variantSkus) &&
        state.variantSkus.length === validVariantCount
      );
    }
    default:
      return false;

...
import { COMMON_COLORS, getColorHex } from "./color-utils";

export const WIZARD_STEPS = [
  { num: 1, label: "Details & Media", slug: "step-1" },
  { num: 2, label: "Variants",        slug: "step-2" },
  { num: 3, label: "Specs & Labels",  slug: "step-3" },
  { num: 4, label: "Discounts",       slug: "step-4" },
  { num: 5, label: "Review",          slug: "step-5" },
];
...
export const COLORS_LIST = COMMON_COLORS;
...
export function getColorTw(name) {
  const color = COLORS_LIST.find(c => c.name === name);
  if (color?.family === "Multi") return "bg-gradient-to-br from-purple-500 via-pink-500 to-amber-400";
  switch (color?.family || name) {
    ...
    case "Beige": return "bg-stone-200 border border-stone-300";
...
export function getColorSwatch(name, hex) {
  const resolved = hex || COLORS_LIST.find(c => c.name === name)?.hex || getColorHex(name);
  return resolved || "#9CA3AF";
}
...
export function requiresComplianceForCategory(category) {
  return ["men", "women", "kids"].includes(String(category || "").trim().toLowerCase());
}

function hasFilledFiberComposition(fiberComposition = []) {
  const filled = fiberComposition.filter((fiber) => fiber?.type || fiber?.percent);
  if (filled.some((fiber) => !fiber?.type || !fiber?.percent)) return false;
  if (filled.length === 0) return true;
  const totalPercent = filled.reduce((sum, fiber) => sum + (parseInt(fiber.percent, 10) || 0), 0);
  return totalPercent === 100;
}

function hasValidVariants(variants = []) {
  if (!variants.length) return false;

  const seen = new Set();
  for (const variant of variants) {
    if (!variant?.color || !variant?.size || variant.quantity <= 0 || variant.price <= 0) {
      return false;
    }

    const key = `${variant.color.trim().toLowerCase()}__${variant.size.trim().toLowerCase()}`;
...
function hasValidBulkDiscountTiers(tiers = []) {
  if (!Array.isArray(tiers)) return false;
  if (!tiers.length) return true;

  const normalized = [];
  for (const tier of tiers) {
    const minimumQuantity = Number.parseInt(tier?.minimum_quantity, 10);
    const discountPercent = Number(tier?.discount_percent);

    if (!Number.isFinite(minimumQuantity) || minimumQuantity < 2) return false;
    if (!Number.isFinite(discountPercent) || discountPercent <= 0 || discountPercent >= 100) return false;

    normalized.push({
      minimum_quantity: minimumQuantity,
      discount_percent: Math.round((discountPercent + Number.EPSILON) * 100) / 100,
    });
  }

  normalized.sort((a, b) => a.minimum_quantity - b.minimum_quantity);

  for (let i = 1; i < normalized.length; i += 1) {
    if (normalized[i].minimum_quantity === normalized[i - 1].minimum_quantity) return false;
    if (normalized[i].discount_percent <= normalized[i - 1].discount_percent) return false;
  }

  return true;
}

function hasValidImages(state) {
  const hasImage = (key) => state?.images?.[key] || state?.persistedImages?.[key];
  const uniqueColors = [...new Set((state?.variants || []).map((variant) => variant.color).filter(Boolean))];

  if (state?.imageStrategy === "general") {
    return Boolean(hasImage("general_front") && hasImage("general_back"));
  }

  if (state?.imageStrategy === "variant") {
    return uniqueColors.length > 0 && uniqueColors.every((color) => {
      const safeColor = color.replace(/\s/g, "_");
      return hasImage(`variant_${safeColor}_front`) && hasImage(`variant_${safeColor}_back`);
    });
  }

  if (state?.imageStrategy === "mixed") {
    return Boolean(hasImage("mixed_general_front") && hasImage("mixed_general_back"));
  }

  return false;
}

function hasSpecificationContent(state) {
  const summary = String(state?.specificationSummary || "").trim();
  const structured = Array.isArray(state?.specifications)
    ? state.specifications.some((entry) => String(entry?.key || "").trim() && String(entry?.value || "").trim())
    : false;

  return Boolean(summary || structured);
}

function hasCompleteCompliance(state) {
  if (!requiresComplianceForCategory(state?.category)) return true;

  const countryOfOrigin = String(state?.countryOfOrigin || "").trim();
  const hasCareInstruction = Boolean(
    state?.careWashing ||
    state?.careBleaching ||
    state?.careDrying ||
    state?.careIroning ||
    state?.careDryCleaning
  );

  return Boolean(
    hasFilledFiberComposition(state?.fiberComposition || []) &&
    countryOfOrigin &&
    hasCareInstruction
  );
}
...
export function getPublishReadiness(state) {
  ...
  if (!(state?.category && state?.subcategory)) blockers.push("Choose a category and subcategory.");
  if (!state?.productName?.trim()) blockers.push("Add a product name.");
  if (!state?.material) blockers.push("Select a material.");
  if (!state?.description?.trim() || state.description.trim().length < 10) blockers.push("Write a fuller product description.");
  if (!hasValidVariants(state?.variants || [])) blockers.push("Add at least one valid size and color variant.");
  if (!hasValidBulkDiscountTiers(state?.bulkDiscountTiers || [])) blockers.push("Fix bulk discount tiers so quantity thresholds and percentages are valid.");
  if (!hasValidImages(state)) blockers.push("Upload the required front and back media.");
  if (!hasSpecificationContent(state)) blockers.push("Add buyer-facing specifications or a specification summary.");
  if (!hasCompleteCompliance(state)) blockers.push("Complete apparel label and care information before publishing.");
...
export function isWizardStepComplete(stepNum, state) {
  switch (stepNum) {
    case 1:
      return Boolean(
        state?.category && state?.subcategory && 
        state?.productName?.trim() && state?.material && state?.description?.trim() && state.description.trim().length >= 10 &&
        hasValidImages(state)
      );
    case 2:
      return hasValidVariants(state?.variants || []);
    case 3:
      return hasSpecificationContent(state);
    case 4: {
      return hasValidBulkDiscountTiers(state?.bulkDiscountTiers || []);
    }
    case 5: {
      return Boolean(state?.baseSku && hasValidVariants(state?.variants || []));
    }
    default:
      return false;
  }

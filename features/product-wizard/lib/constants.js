...
import { getGenderValidationError } from "./category-gender-rules";

export const WIZARD_STEPS = [
  { num: 1, label: "Details & Media", slug: "step-1" },
  { num: 2, label: "Variants",        slug: "step-2" },
  { num: 3, label: "Compliance",      slug: "step-3" },
...
export const SUBCATEGORIES = {
  ...
  kids:     ["Tops", "Bottoms", "Dresses", "Sets", "Outerwear", "Sleepwear", "Underwear", "School Uniforms"],
...
function hasValidImages(state) {
  const allImageKeys = Object.keys({
    ...(state?.images || {}),
    ...(state?.persistedImages || {}),
  });
  const productMediaCount = allImageKeys.filter((key) => key.startsWith("general_")).length;
  return productMediaCount >= 2;
}
...
export function getPublishReadiness(state) {
  ...
  const genderError = getGenderValidationError(state?.category, state?.gender);
  if (genderError) blockers.push(genderError);
  ...
  if (!hasValidImages(state)) blockers.push("Upload at least 2 product images.");
  // Step 3 is optional and should never block publishing.
  ...
  switch (stepNum) {
    case 1:
      return Boolean(
        ...
        state?.productName?.trim() && state?.description?.trim() && state.description.trim().length >= 10 &&
...
function hasOptionalComplianceContent(state) {
  return Boolean(
    String(state?.specificationSummary || "").trim() ||
    String(state?.countryOfOrigin || "").trim() ||
    String(state?.countryOfTransformation || "").trim() ||
    state?.careWashing ||
    state?.careBleaching ||
    state?.careDrying ||
    state?.careIroning ||
...
function hasConfiguredBulkDiscounts(state) {
  return Array.isArray(state?.bulkDiscountTiers) && state.bulkDiscountTiers.length > 0;
}

export function isWizardStepFilled(stepNum, state) {
  switch (stepNum) {
    case 1:
      return isWizardStepComplete(1, state);
    case 2:
      return isWizardStepComplete(2, state);
    case 3:
      return hasOptionalComplianceContent(state);
    case 4:
      return hasConfiguredBulkDiscounts(state);
    case 5:
      return false;
    default:
      return false;
...
export function getWizardStepUiState(state, currentStep) {
  const steps = WIZARD_STEPS.map((step) => {
    const active = step.num === currentStep;
    const filled = isWizardStepFilled(step.num, state);
    // Allow users to always navigate back to earlier steps, even optional/incomplete ones.
    const clickable = !active && (step.num < currentStep || filled);
    return {
      ...step,
      active,
      filled,
  ...
  const filledCount = steps.filter((step) => step.filled).length;
...
export const MOODS = [
  {
    ...
    label: 'Owambe / Party',
    emoji: '🎉',
    desc: 'Ankara, aso-ebi, lace, celebrations',
  },
  {
    key: 'casual_chill',
    label: 'Casual & Chill',
    emoji: '😎',
    desc: 'Everyday relaxed fits, joggers, basics',
  },
  {
    key: 'office_ready',
    label: 'Office Ready',
    emoji: '💼',
    desc: 'Corporate, formal, workwear',
  },
  {
    key: 'date_night',
    label: 'Date Night',
    emoji: '🌙',
    desc: 'Evening, elegant, romantic looks',
  },
  {
    key: 'sunday_best',
    label: 'Sunday Best',
    emoji: '⛪',
    desc: 'Church wear, modest, conservative',
  },
  {
    key: 'street_trendy',
    label: 'Street / Trendy',
    emoji: '🛹',
    desc: 'Urban streetwear, oversized, drip',
  },
  {
    key: 'soft_luxury',
    label: 'Soft Luxury',
    emoji: '✨',
    desc: 'Premium, elevated, tailored minimalism',
  },
  {
    key: 'travel_weekend',
    label: 'Travel & Weekend',
    emoji: '✈️',
    desc: 'Lightweight, comfortable, outing-ready',
  },
];
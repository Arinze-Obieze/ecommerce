...
export async function GET(request) {
  ...
  try {
    ...
    let limit = toPositiveInt(searchParams.get('limit'), DEFAULT_LIMIT, { min: 1, max: MAX_LIMIT });

    const offset = (page - 1) * limit;

    const category = sanitizeSlug(searchParams.get('category'));
    ...
    return errorJson('Failed to fetch products');
  }
}
